...
export async function POST(request: NextRequest) {
    try {
      ...
      const validItems = items.filter(item => {
        ...
        return item.item_id && item.sl_id && item.quantity && item.unit_id;
      });
  
      if (validItems.length !== items.length) {
        return NextResponse.json({ error: "Some items have missing fields (item_id, sl_id, quantity, or unit_id)" }, { status: 400 });
      }
  
      // Ensure the unit exists or create it if doesn't
      for (const item of validItems) {
        ...
        let unitRecord = await prisma.unit.findFirst({
          where: {
            unit_name: {
              equals: item.unit_name,  // Compare unit_name
        ...
        // Update the item's unit_id with the created or found unit's ID
      ...
      const inventoryShelfData = validItems.map(item => {
        ...
        return {
          ...
          unit_id: item.unit_id, // Unit ID must be properly set
        };
      });
